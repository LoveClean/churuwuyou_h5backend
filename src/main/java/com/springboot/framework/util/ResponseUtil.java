...
import com.springboot.framework.constant.HttpStatusCode;
...
public class ResponseUtil {
  ...
  public static <T> ResponseBO<T> addMessage(int num){
	  return num == 0 ?
			  ResponseUtil.fail(Errors.SYSTEM_INSERT_FAIL) :
			  ResponseUtil.success();
  }
  
  public static <T> ResponseBO<T> message(T data){
	  return data == null ?
			  ResponseUtil.fail(Errors.SYSTEM_DATA_NOT_FOUND) :
			  ResponseUtil.success(data);
  }
  
  public static <T> ResponseBO<T> updMessage(int num){
	  return num == 0 ?
			  ResponseUtil.fail(Errors.SYSTEM_UPDATE_ERROR) :
			  ResponseUtil.success();
  }
  
  public static <T> ResponseBO<T> delMessage(int num){
	  return num == 0 ?
			  ResponseUtil.fail(Errors.SYSTEM_DELETE_FAIL) :
			  ResponseUtil.success();
  }
}
